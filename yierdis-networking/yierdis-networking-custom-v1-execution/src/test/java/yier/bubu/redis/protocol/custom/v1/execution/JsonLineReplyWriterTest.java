...
public class JsonLineReplyWriterTest {
    ...
    public void replyWriterAuthorityMustStayDocumentedInReadmeAndProtocolSources() throws Exception {
        ...
        String replyValueSource = Files.readString(
                workspaceRoot.resolve("yierdis-networking/yierdis-networking-custom-v1/src/main/java/yier/bubu/redis/protocol/custom/v1/reply/ReplyValue.java"),
                StandardCharsets.UTF_8
        );
        Assert.assertTrue(replyValueSource.contains("server 命令执行写回仍以 {@code ReplyWriter} 为准"));

        String encoderSource = Files.readString(
                workspaceRoot.resolve("yierdis-networking/yierdis-networking-custom-v1/src/main/java/yier/bubu/redis/protocol/custom/v1/wire/CustomProtocolV1NdjsonEncoder.java"),
                StandardCharsets.UTF_8
        );
        Assert.assertTrue(
    ...
    private static Path resolveWorkspaceRoot() {
        ...
        while (cursor != null) {
            if (Files.isRegularFile(cursor.resolve("README.md"))
                    && Files.isDirectory(cursor.resolve("yierdis-server/yierdis-server-main/src/main/java"))
                    && Files.isDirectory(cursor.resolve("yierdis-networking/yierdis-networking-custom-v1/src/main/java"))
                    && Files.isDirectory(cursor.resolve("yierdis-networking/yierdis-networking-custom-v1-execution/src/main/java"))) {
                return cursor;
            }
