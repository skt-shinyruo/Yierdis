...
- **协议模型（limits/build-info/reply tooling/client/parser model）**：继续位于 `yierdis-protocol-model`（包名 `yier.bubu.redis.protocol.*`）；其中 `ReplyValue` 仅用于协议侧客户端/工具/解析器与编码辅助，server 命令写回语义仍以 `ReplyWriter` 为单一事实来源。
