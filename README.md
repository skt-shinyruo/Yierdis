...
- **off-heap contract 兼容面**：`OffHeapAllocator` / `OffHeapBuf` / `OffHeapSlice` / `OffHeapOutOfMemoryException` 位于 `yierdis-memory-api`；包名仍为 `yier.bubu.redis.offheap.api`，需要这些 contract 的模块应直接依赖 `yierdis-memory-api`。
