...
- 移除 deprecated bytes alias（`YierdisBytesSink/YierdisBytesSource/YierdisDirectBytesSink` 等），bytes SSOT jar 统一为 `yierdis-bytes-lib`（`yierdis-bytes` 为父 POM/聚合层）。
...
- protocol 模块重构：`yierdis-protocol` 从 jar 兼容聚合层调整为 pom 父 POM（multi-module parent），并将 `yierdis-protocol-model` / `yierdis-protocol-codec` / `yierdis-protocol-netty` 迁入 `yierdis-protocol/` 目录下作为子模块；依赖方应直接依赖具体子模块（版本由 `yierdis-parent` 的 `dependencyManagement` 统一管理）。
...
- 新增中立 bytes 模块：以 `yierdis-bytes-lib` 承载 `BytesSource/BytesSink/BytesSlice` 抽象（由 `yierdis-bytes` 聚合），供 protocol/off-heap/I/O 复用，避免通过 “off-heap” 命名模块复用 bytes 接口造成依赖误导。
...
- core 进一步模块化拆分：新增 `yierdis-core-api`（ops/stable types/ports，Netty-free）、`yierdis-core-db`（db impl）、`yierdis-core-command`（command impl）、`yierdis-core-runtime`（instance/runtime 装配）；`yierdis-core` 调整为父 POM/聚合层（multi-module parent），core 测试与 `yierdis-version.properties` 资源迁移到 `yierdis-core-runtime`。
...
- Maven 多模块拆分：引入协议层模块（`yierdis-protocol-model` 端口/模型 SSOT + `yierdis-protocol-codec` JSON/v1 codec SSOT；`yierdis-protocol` 为父 POM/聚合层）、core 模块族（`yierdis-core-*`，由 `yierdis-core` 聚合）、`yierdis-args`（参数 SSOT）、`yierdis-client`（client/CLI），并调整 `yierdis-server`/`yierdis-bench` 依赖方向。
...
- `yierdis-protocol` 依赖收敛：不再直接依赖 `yierdis-offheap-api`，改为依赖中立 bytes SSOT jar（`yierdis-bytes-lib`）；同时移除 `yierdis-offheap-api` 的 bytes 兼容别名，避免 SSOT 漂移。
- protocol 依赖方向收敛：`yierdis-core-*`/`yierdis-args` 依赖 `yierdis-protocol-model`（避免编译期引入 codec）；`yierdis-protocol-netty` 依赖 `yierdis-protocol-codec`；`yierdis-protocol` 为父 POM/聚合层以降低迁移成本。
...
- off-heap：core 通过 `YierdisOffHeapAddressAllocator` capability 选择 keyspace/expires 的 off-heap 路径，避免对具体后端类型的 `instanceof` 耦合；core 模块族不再编译期依赖 `yierdis-offheap-unsafe`。
...
### Fixed
- Maven：根 `dependencyManagement` 补齐 `yierdis-core-*` 版本管理，并移除全仓重复的 `<version>${project.version}</version>` 声明，降低版本漂移风险。
- Maven：根 `dependencyManagement` 纳入 `yierdis-offheap-*` / `yierdis-args` / `yierdis-executor-core` 的版本管理（含 `yierdis-offheap-api:tests` classifier），并清理对应依赖里的重复版本声明，降低版本漂移风险。
...
- bench 依赖更新：压测工具写请求时改用中立 bytes SSOT（`yierdis-bytes-lib`）的 `BytesSink`（避免依赖已移除的包路径）。
