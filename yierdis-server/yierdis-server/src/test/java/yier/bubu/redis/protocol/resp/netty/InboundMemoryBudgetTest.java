...
public class InboundMemoryBudgetTest {
    ...
    public void releasesBackpressureInStrictFifoOrder() {
        ...
        InboundConnectionMemory a = connection(100, () -> { });
        InboundConnectionMemory b = connection(100, resumedB::incrementAndGet);
        InboundConnectionMemory c = connection(100, resumedC::incrementAndGet);
    ...
    public void closesGrantedReservationBeforeItsEventLoopCallbackRuns() {
        ...
        InboundConnectionMemory a = connection(100, () -> { });
        InboundConnectionMemory b = new InboundConnectionMemory(100, pendingCallbacks::add, () -> { });
    ...
    public void requestThatCannotEverFitIsRejectedRatherThanQueued() {
        InboundMemoryBudget budget = new InboundMemoryBudget(100);
        InboundConnectionMemory connection = connection(100, () -> { });
    ...
    public void transferAdmitsTemporaryCopyPeakBeforeInputRelease() {
        ...
        Assert.assertEquals(80L, connection.account().reservedBytes());
    }
    ...
    public void transferWaitsOnlyForCapacityAnotherConnectionCanRelease() {
        ...
        InboundConnectionMemory source = connection(100, resumed::incrementAndGet);
        InboundConnectionMemory blocker = connection(100, () -> { });
    ...
    public void currentRequestProgressBypassesOnlyTheSoftHighWatermark() {
        InboundMemoryBudget budget = new InboundMemoryBudget(100);
        InboundConnectionMemory current = connection(200, () -> { });
        InboundConnectionMemory fresh = connection(100, () -> { });
        InboundConnectionMemory queued = connection(100, () -> { });
    ...
    public void progressWaiterCanAdvanceAheadOfNewWorkWhileBackpressured() {
        ...
        InboundConnectionMemory current = connection(100, progressWakeups::incrementAndGet);
        InboundConnectionMemory holder = connection(100, () -> { });
        InboundConnectionMemory fresh = connection(100, freshWakeups::incrementAndGet);
    ...
    public void closingBudgetCancelsWaitersButAllowsLateLeaseRelease() {
        InboundMemoryBudget budget = new InboundMemoryBudget(100);
        InboundConnectionMemory active = connection(100, () -> { });
        InboundConnectionMemory waiting = connection(100, () -> { });
    ...
    public void rejectsInvalidCapacitiesAmountsCreditsAndForeignAccounts() {
        ...
        Assert.assertThrows(IllegalArgumentException.class,
                () -> new InboundConnectionMemory(-1, Runnable::run, () -> { }));
        Assert.assertThrows(NullPointerException.class,
                () -> new InboundConnectionMemory(1, null, () -> { }));
        Assert.assertThrows(NullPointerException.class,
                () -> new InboundConnectionMemory(1, Runnable::run, null));

        InboundMemoryBudget budget = new InboundMemoryBudget(10);
        InboundConnectionMemory connection = connection(10, () -> { });
    ...
    public void closedQueuedAccountIsSkippedWhenAnotherWaiterCanBeGranted() {
        ...
        AtomicInteger resumed = new AtomicInteger();
        InboundConnectionMemory blocker = connection(100, () -> { });
        InboundConnectionMemory closed = connection(100, () -> { });
        InboundConnectionMemory live = connection(100, resumed::incrementAndGet);
    ...
    public void countersSaturateRejectUnderflowAndZeroCapacityHasDefinedBehavior() {
        ...
        InboundConnectionMemory connection = connection(0, () -> { });
    ...
    private static InboundConnectionMemory connection(long hardLimit, Runnable onResume) {
        return new InboundConnectionMemory(hardLimit, Runnable::run, onResume);
    }
}
