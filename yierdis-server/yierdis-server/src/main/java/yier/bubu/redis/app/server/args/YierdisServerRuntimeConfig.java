...
public record YierdisServerRuntimeConfig(
        ...
        long maxmemoryBytes,
        YierdisInstanceConfig.MaxmemoryScope maxmemoryScope,
    ...
    public YierdisServerRuntimeConfig {
        ...
        requireRange(protocolMaxBulkBytes, 1, RespProtocolLimits.DEFAULT_MAX_BULK_BYTES, "protocolMaxBulkBytes");
        requireRange(protocolMaxArgs, 1, RespProtocolLimits.DEFAULT_MAX_ARGS, "protocolMaxArgs");
    ...
    public static YierdisInstanceConfig.MaxmemoryScope parseMaxmemoryScope(String rawValue) {
        ...
        String normalized = rawValue.trim().toLowerCase(java.util.Locale.ROOT).replace('_', '-');
        if ("perdb".equals(normalized)) {
            normalized = "per-db";
        }
        return switch (normalized) {
            case "global" -> YierdisInstanceConfig.MaxmemoryScope.GLOBAL;
            case "per-db" -> YierdisInstanceConfig.MaxmemoryScope.PER_DB;
