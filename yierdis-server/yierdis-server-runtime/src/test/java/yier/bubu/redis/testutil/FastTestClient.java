...
public final class FastTestClient implements AutoCloseable {
    ...
    public ReplyObject execute(ExecutionRequest request) {
        ...
        try (request;
             PreparedCommand prepared = processor.prepare(
                     request,
                     new CommandPreparationContext(session)
             )) {
            if (prepared.validateBeforeExecute() != ValidationResult.VALID) {
                throw new AssertionError("test client cannot execute a stale prepared command");
            }
            try (CommandExecutionContext context = CommandExecutionContext.forRequest(session, writer, request)) {
