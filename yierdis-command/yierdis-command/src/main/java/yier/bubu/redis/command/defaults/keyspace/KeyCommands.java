...
public final class KeyCommands {
    ...
    private Function<CommandSession, PreparedCommand> type(CommandArgs args) {
        BytesSlice key = args.slice(1);
        return session -> {
            ValueType valueType = support.commandDb(session).keyspace().typeOf(key);
    ...
    private Function<CommandSession, PreparedCommand> memory(CommandArgs args) {
        ...
        if (args.is(1, "STATS")) {
            ...
            return session -> {
                ...
                if (stats == null) {
                    stats = support.commandDb(session).memoryStats();
                }
    ...
    private Function<CommandSession, PreparedCommand> keys(CommandArgs args) {
        ...
        return session -> {
            SlowCommandLimits limits = support.slowCommandLimits();
            long timeBudgetNanos = limits.keysTimeBudgetNanos();
            ...
            for (int attempt = 0; attempt < KEY_WINDOW_DISCOVERY_ATTEMPTS; attempt++) {
                ...
                KeyScanWindow window = support.commandDb(session).keyspace().keys(
                        pattern,
                        limits.keysMaxResults(),
    ...
    private Function<CommandSession, PreparedCommand> del(CommandArgs args) {
        List<byte[]> keys = args.byteArraysFrom(1);
        return session -> CommandSupport.preparedAction(ReplyShapes.integerUpperBound(), execution -> {
            long deleted = support.commandDb(execution).keyspace().del(keys).value();
            return CommandResult.reply(RedisReplies.integer(deleted));
        });
    }

    private Function<CommandSession, PreparedCommand> exists(CommandArgs args) {
        ...
        return session -> {
            ...
            for (BytesSlice key : keys) {
                if (support.commandDb(session).keyspace().existsKey(key)) {
                    count++;
                }
            }
    ...
    private Function<CommandSession, PreparedCommand> expireat(CommandArgs args) {
        TtlArgs parsed = ttlArgs(args);
        return session -> CommandSupport.preparedAction(ReplyShapes.integerUpperBound(), execution -> {
    ...
    private Function<CommandSession, PreparedCommand> pexpireat(CommandArgs args) {
        TtlArgs parsed = ttlArgs(args);
        return session -> CommandSupport.preparedAction(ReplyShapes.integerUpperBound(), execution -> {
            boolean applied = support.commandDb(execution).ttl()
    ...
    private Function<CommandSession, PreparedCommand> persist(CommandArgs args) {
        BytesSlice key = args.slice(1);
        return session -> CommandSupport.preparedAction(ReplyShapes.integerUpperBound(), execution -> {
            boolean applied = support.commandDb(execution).ttl().persist(key).value();
            return CommandResult.reply(RedisReplies.integer(applied ? 1L : 0L));
        });
    }

    private Function<CommandSession, PreparedCommand> ttl(CommandArgs args) {
        BytesSlice key = args.slice(1);
        return session -> PreparedCommands.ready(RedisReplies.integer(
                support.commandDb(session).ttl().ttlSeconds(key)));
    }

    private Function<CommandSession, PreparedCommand> pttl(CommandArgs args) {
        BytesSlice key = args.slice(1);
        return session -> PreparedCommands.ready(RedisReplies.integer(
                support.commandDb(session).ttl().ttlMillis(key)));
    }
