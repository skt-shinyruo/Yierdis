...
public final class ListCommands {
    ...
    private Function<CommandSession, PreparedCommand> lrange(CommandArgs args) {
        byte[] key = args.bytes(1);
        int start = args.intClampedAt(2);
        int stop = args.intClampedAt(3);
        return session -> {
            ByteSequenceSource values = support.commandDb(session).lists().lrange(key, start, stop);
