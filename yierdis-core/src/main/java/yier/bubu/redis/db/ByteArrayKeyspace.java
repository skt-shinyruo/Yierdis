...
final class ByteArrayKeyspace<V> implements YierdisKeyspace<V> {
    ...
    private static int findIndex(byte[][] keys, int[] hashes, byte[] states, YierdisBytesView key, int hash) {
        int mask = keys.length - 1;
        int idx = hash & mask;
        while (true) {
            byte state = states[idx];
    ...
    private static boolean equals(byte[] storedKey, YierdisBytesView key) {
        int len = key.len();
