...
public final class YierdisUnsafeOffHeapExpireIndex implements YierdisExpireIndex {
    ...
    public YierdisUnsafeOffHeapExpireIndex(OffHeapAddressAllocator allocator) {
    ...
    private Location findLocationByContent(BytesView keyView) {
        Table t0 = table0;
        if (t0 == null) {
            return null;
        }
    ...
    private int findIndex(Table t, BytesView key, int hash) {
        int mask = t.capacity - 1;
        int idx = hash & mask;
        int keyLen = key.len();
        while (true) {
    ...
    private boolean equalsKey(long storedPtr, BytesView key, int len) {
        for (int i = 0; i < len; i++) {
    ...
    private final class Table {
        ...
        final OffHeapBlock expireAtBlock;
