 ...
 * <p>
 * This implementation uses Redis-style incremental rehashing to avoid latency spikes.
 * It is intentionally minimal and optimized for low object overhead.
 * It is <b>not</b> thread-safe.
 */
public final class ByteArrayHashMap<V> {
    // Approximate per-reference bytes used by Java object arrays for teaching/demo accounting.
    // This is intentionally a stable constant rather than trying to infer VM details.
    ...
    public ByteArrayHashMap(int expectedSize) {
    ...
    public long estimatedBytes() {
        long bytes = estimateTableBytes(keys0, hashes0, values0, states0);
        if (keys1 != null) {
            bytes += estimateTableBytes(keys1, hashes1, values1, states1);
        }
        return bytes;
    }

    private static long estimateTableBytes(byte[][] keys, int[] hashes, Object[] values, byte[] states) {
        if (keys == null) {
            return 0;
        }
        // Primitive arrays (real bytes) + object reference arrays (approximate).
        return (long) hashes.length * Integer.BYTES
                + (long) states.length * Byte.BYTES
                + (long) keys.length * REF_BYTES
                + (long) values.length * REF_BYTES;
    }
    ...
    public boolean containsKey(byte[] key) {
        ...
        rehashStep();
        int h = hash(key);
        return findIndex(keys0, hashes0, states0, key, h) >= 0
                || (keys1 != null && findIndex(keys1, hashes1, states1, key, h) >= 0);
    }
    ...
    public V put(byte[] key, V value) {
        ...
        if (loc != null) {
            @SuppressWarnings("unchecked")
            V old = (V) loc.getValue();
            loc.setValue(value);
            return old;
        }

        insertNew(key, h, value);
        return null;
    }
    ...
    private final class Location {
        ...
        Object getValue() {
            return table == 0 ? values0[index] : values1[index];
        }
