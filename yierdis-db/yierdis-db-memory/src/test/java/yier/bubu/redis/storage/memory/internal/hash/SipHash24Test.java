...
public class SipHash24Test {
    ...
    private static final long[] REFERENCE_VECTORS = {
    ...
    @Test
    public void matchesOfficialSipHash24Vectors() {
        byte[] message = new byte[64];
        for (int i = 0; i < message.length; i++) {
            message[i] = (byte) i;
        }

        for (int length = 0; length < REFERENCE_VECTORS.length; length++) {
            byte[] input = new byte[length];
            System.arraycopy(message, 0, input, 0, length);
            Assert.assertEquals("vector " + length, REFERENCE_VECTORS[length], SipHash24.hash(REFERENCE_SEED, input));
        }
    }

    @Test
    public void hashesByteArrayBytesViewAndNativeViewIdenticallyForBinaryData() {
        ...
        try (YierdisFfmMemoryRuntime runtime = new YierdisFfmMemoryRuntime("siphash-test");
            ...
            try {
                try (NativeObjectView view = allocator.resolve(handle, NativeAccessMode.READ_WRITE)) {
                    view.setBytes(0, input, 0, input.length);
                }
                try (NativeObjectView view = allocator.resolve(handle, NativeAccessMode.READ_ONLY)) {
                    Assert.assertEquals(expected, SipHash24.hash(REFERENCE_SEED, view));
                }
            } finally {
