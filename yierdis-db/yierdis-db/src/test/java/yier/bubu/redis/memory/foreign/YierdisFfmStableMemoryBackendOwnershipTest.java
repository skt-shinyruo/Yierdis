...
public class YierdisFfmStableMemoryBackendOwnershipTest {
    ...
    public void ownershipIsCheckedBeforeMalformedLocalRaw() {
        ...
        try (StableMemoryBackend backend = backend("order", owner)) {
            ...
            Assert.assertTrue(failure.getMessage().contains(Long.toString(backend.allocatorId())));
            Assert.assertTrue(failure.getMessage().contains(Long.toString(foreignMalformed.allocatorId())));
        }
    }
    ...
    private static void assertOwnershipFailure(StableMemoryBackend backend, NativeHandle foreign) {
        ...
        Assert.assertThrows(NativeMemoryException.class, () -> backend.free(foreign));
        Assert.assertThrows(NativeMemoryException.class, () -> backend.pin(foreign));
        Assert.assertThrows(NativeMemoryException.class, () -> backend.unpin(foreign));
    }
