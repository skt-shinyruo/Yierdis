...
public class YierdisDbKeyLifecycleTest {
    ...
    public void inspectionAndNullInputsHaveStableNoopContracts() {
        withDb(db -> {
            ...
            Assert.assertNull(lifecycle.entryRecord((AllocatorKeyHandle) null));
            ...
            Assert.assertNull(lifecycle.liveEntryRecord((AllocatorKeyHandle) null));
            Assert.assertNull(lifecycle.copyKeyBytes(null));
            Assert.assertNull(lifecycle.expireAtMillis((byte[]) null));
            Assert.assertNull(lifecycle.expireAtMillis((AllocatorKeyHandle) null));
            ...
            lifecycle.resetExpirationTracking();
    ...
    public void stagedEntryTokenAbortsOrPublishesOwnedHandlesExactlyOnce() {
        withDb(db -> {
            ...
            Assert.assertEquals(baselineLiveObjects + 2L,
                    KeyLifecycleTestAccess.backend(db).stats().liveObjects());
            aborted.close();
            aborted.close();
            Assert.assertEquals(baselineLiveObjects,
                    KeyLifecycleTestAccess.backend(db).stats().liveObjects());
    ...
    public void ttlUpdatesReplaceEntryRecordsAndKeepDerivedCountExact() {
        withDb(db -> {
            ...
            Assert.assertTrue(db.ttl().expireAtMillis(view(key), deadline).value());
            ...
            Assert.assertTrue(db.ttl().persist(view(key)).value());
    ...
    public void expectedRecordChecksAndUnlinkVariantsReleaseResourcesOnce() {
        withDb(db -> {
            ...
            AllocatorKeyHandle keyHandle = lifecycle.keyHandle(firstKey);
