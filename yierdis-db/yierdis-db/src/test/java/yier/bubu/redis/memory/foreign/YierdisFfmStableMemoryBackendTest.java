...
public class YierdisFfmStableMemoryBackendTest {
    ...
    public void largeReallocAndDefragPreserveContentAndStableHandle() {
        try (YierdisFfmMemoryRuntime runtime = new YierdisFfmMemoryRuntime("stable-large-copy");
            ...
            NativeDefragReport result = allocator.defragCycle(
                    new NativeDefragOptions(expanded.length, 1, Long.MAX_VALUE)
            );

            Assert.assertEquals(1L, result.movedObjects());
    ...
    public void defragMovesUnpinnedObjectWithoutChangingHandle() {
        try (YierdisFfmMemoryRuntime runtime = new YierdisFfmMemoryRuntime("stable-test");
            ...
            NativeDefragReport result = allocator.defragCycle(new NativeDefragOptions(24, 1, Long.MAX_VALUE));
    ...
    public void defragSkipsPinnedAndOverBudgetObjects() {
        try (YierdisFfmMemoryRuntime runtime = new YierdisFfmMemoryRuntime("stable-test");
            ...
            try {
                NativeDefragReport pinned = allocator.defragCycle(new NativeDefragOptions(24, 1, Long.MAX_VALUE));
                Assert.assertEquals(0L, pinned.movedObjects());
                Assert.assertEquals(1L, pinned.skippedPinnedObjects());
            ...
            NativeDefragReport budget = allocator.defragCycle(new NativeDefragOptions(23, 1, Long.MAX_VALUE));
            Assert.assertEquals(0L, budget.movedObjects());
            Assert.assertTrue(budget.stoppedByByteBudget());
            Assert.assertEquals(location, locationOf(allocator.objectMeta(handle.localRaw(), false)));
            allocator.free(handle);
        }
    }
        ...
        try (YierdisFfmMemoryRuntime runtime = new YierdisFfmMemoryRuntime("stable-churn");
            ...
            for (int i = 0; i < 180; i++) {
                ...
                if (op < 55) {
                    ...
                    try {
                        NativeDefragReport result = allocator.defragCycle(
                                new NativeDefragOptions(Long.MAX_VALUE, Long.MAX_VALUE, Long.MAX_VALUE)
                        );
                        Assert.assertTrue(result.skippedPinnedObjects() > 0L);
                    } finally {
                        allocator.unpin(handle);
                    }
                } else if (op < 95) {
                    allocator.defragCycle(new NativeDefragOptions(Long.MAX_VALUE, Long.MAX_VALUE, Long.MAX_VALUE));
                } else {
                    allocator.free(handle);
                    live.remove(index);
                    try {
