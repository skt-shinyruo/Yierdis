...
final class YierdisHllOps implements HllOps {
    ...
    public WriteResult<Void> pfmerge(byte[] destKeyBytes, List<byte[]> sourceKeys) {
        ...
        return kernel.execute(new MutationPlan<WriteResult<Void>>() {
            @Override
            public long upperBoundBytes() {
            ...
            public PreparedDbMutation<WriteResult<Void>> prepare() {
                ...
                try {
                    ...
                    long deltaBytes = estimateRecordBytes(targetKey, next)
                            - estimateRecordBytes(targetKey, current);
                    PreparedDbMutation<WriteResult<Void>> prepared = kernel.upsert(
                            result,
                            deltaBytes,
                            staged == null ? 0L : staged.stagedHeapBytes(),
                            currentEntry,
    ...
    private EntryRecord hllRecord(AllocatorKeyHandle keyHandle, ValueHandle handle, long expireAtMillis, EntryRecord previous) {
