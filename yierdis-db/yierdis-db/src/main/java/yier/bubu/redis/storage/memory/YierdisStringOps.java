...
final class YierdisStringOps implements StringOps {
    ...
    private WriteResult<SetStringValue> setInternal(
        ...
        return kernel.execute(new MutationPlan<WriteResult<SetStringValue>>() {
            @Override
            ...
            @Override
            public PreparedDbMutation<WriteResult<SetStringValue>> prepare() {
                ...
                if (mode == SetMode.NX && current != null) {
                    WriteResult<SetStringValue> result = WriteResult.unchanged(
                            new SetStringValue(false, ByteValue.NULL)
                    );
                    return kernel.unchanged(result);
                }
                ...
                try {
                    ...
                    PreparedEntryMutation<WriteResult<SetStringValue>> prepared = kernel.upsert(
                            result,
                            deltaBytes,
            ...
            public PreparedDbMutation<WriteResult<Long>> prepare() {
                ...
                try {
                    ...
                    if (current == null) {
                        ...
                        if (suffix.length == 0) {
                            return kernel.unchanged(WriteResult.unchanged((long) beforeLen));
                        }
            ...
            public PreparedDbMutation<WriteResult<Integer>> prepare() {
                ...
                try {
                    ...
                    if (existed && oldBit == value && before.length >= requiredLen) {
                        return kernel.unchanged(WriteResult.unchanged(oldBit));
                    }
                    ...
                    long deltaBytes = estimateRecordBytes(targetKey, next) - estimateRecordBytes(targetKey, current);
                    PreparedEntryMutation<WriteResult<Integer>> prepared = kernel.upsert(
                            result,
                            deltaBytes,
                            staged == null ? 0L : staged.stagedHeapBytes(),
                            currentEntry,
    ...
    private EntryRecord liveTouchedStringRecord(YierdisDbKernel kernel, byte[] keyBytes) {
        ...
        return liveTouchedStringRecord(kernel, keyHandle);
    }

    private EntryRecord liveTouchedStringRecord(YierdisDbKernel kernel, BytesView keyView) {
        AllocatorKeyHandle keyHandle = keyLifecycle.keyHandle(keyView);
        return liveTouchedStringRecord(kernel, keyHandle);
    }

    private EntryRecord liveTouchedStringRecord(YierdisDbKernel kernel, AllocatorKeyHandle keyHandle) {
    ...
    private long estimateRecordBytes(AllocatorKeyHandle keyHandle, EntryRecord record) {
