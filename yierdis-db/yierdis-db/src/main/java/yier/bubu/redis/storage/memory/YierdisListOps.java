...
final class YierdisListOps implements ListOps {
    ...
    private WriteResult<Long> pushInternal(
        ...
        return kernel.execute(new MutationPlan<WriteResult<Long>>() {
            ...
            public PreparedDbMutation<WriteResult<Long>> prepare() {
                ...
                if (current != null) {
                    requireList(current);
                    return prepareExistingPush(kernel, currentEntry, current, values, left);
                }
                ...
                try {
                    ...
                    PreparedDbMutation<WriteResult<Long>> prepared = kernel.insert(
                            result,
                            deltaBytes,
    ...
    private WriteResult<PoppedValueSequence> popInternal(
        ...
        long now = System.currentTimeMillis();
        kernel.reclaimExpiredBeforeMutation(keyBytes, now);
        return kernel.execute(new MutationPlan<WriteResult<PoppedValueSequence>>() {
            @Override
            public long upperBoundBytes() {
            ...
            public AdmissionMode admissionMode() {
                return isPopReclamation(keyBytes, count, now)
                        ? AdmissionMode.RECLAMATION
                        : AdmissionMode.NORMAL;
            }

            @Override
            public PreparedDbMutation<WriteResult<PoppedValueSequence>> prepare() {
                CurrentEntry currentEntry = keyLifecycle.currentEntry(keyBytes);
                EntryRecord current = currentEntry.record();
                if (current == null) {
    ...
    private PreparedDbMutation<WriteResult<Long>> prepareExistingPush(
        ...
        if (values.isEmpty()) {
            return kernel.unchanged(WriteResult.unchanged((long) listRoot.size(handle)));
        }
    ...
    private EntryRecord liveListRecord(YierdisDbKernel kernel, byte[] keyBytes) {
        AllocatorKeyHandle keyHandle = keyLifecycle.keyHandle(keyBytes);
    ...
    private EntryRecord listRecord(AllocatorKeyHandle keyHandle, ValueHandle handle, long expireAtMillis, EntryRecord previous) {
    ...
    private <T> PreparedDbMutation<T> preparedDelete(
        ...
        PreparedEntryMutation<T> prepared = kernel.delete(
                result,
                deltaBytes,
                currentEntry.entryHandle(),
                current,
