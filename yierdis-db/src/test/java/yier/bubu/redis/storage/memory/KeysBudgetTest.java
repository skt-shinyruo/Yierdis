...
public class KeysBudgetTest {
    ...
    public void keysReturnsPartialResultsWhenTimeBudgetExceeded() {
        ...
        try {
            ...
            for (int i = 0; i < 4096; i++) {
                ...
                db.strings().setString(key, val, SetMode.NORMAL, null);
            }

            try (KeyScanWindow window = db.keyspace().keys(
    ...
    public void keysReturnsPartialResultsWhenResultLimitExceeded() {
        ...
        try {
            for (int i = 0; i < 4; i++) {
                byte[] key = ("k" + i).getBytes(StandardCharsets.US_ASCII);
                db.strings().setString(key, "v".getBytes(StandardCharsets.US_ASCII), SetMode.NORMAL, null);
            }

            try (KeyScanWindow window = db.keyspace().keys("*".getBytes(StandardCharsets.US_ASCII), 1, 0L)) {
    ...
    public void keyWindowReplaysBoundedNativeKeysWithoutRetainingResultLists() {
        ...
        try {
            ...
            for (int i = 0; i < 13; i++) {
                db.strings().setString(bytes("filler-" + i), bytes("v"), SetMode.NORMAL, null);
            }
            ...
            for (byte[] key : List.of(first, second, third)) {
                db.strings().setString(key, bytes("v"), SetMode.NORMAL, null);
            }
            db.strings().setString(expired, bytes("v"), SetMode.NORMAL, ExpireOption.px(0));

            byte[] tombstone = bytes("tombstone");
            db.strings().setString(tombstone, bytes("v"), SetMode.NORMAL, null);
            db.keyspace().del(List.of(tombstone));
            ...
            try (KeyScanWindow window = db.keyspace().keys(bytes("match:*"), 16, 0L)) {
            ...
            KeyScanWindow stale = db.keyspace().scan(ScanCursorV2.start(), bytes("match:*"), 16);
            try {
    ...
    private static String asString(yier.bubu.redis.storage.memory.internal.key.AllocatorKeyHandle key) {
