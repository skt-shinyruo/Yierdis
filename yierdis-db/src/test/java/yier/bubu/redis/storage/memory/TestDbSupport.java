...
public final class TestDbSupport {
    ...
    static WriteResult<StringOps.SetStringValue> commitSetWithOldValue(
            StringOps operations,
            byte[] keyBytes,
            BytesSlice value,
            SetMode mode,
            ExpireOption expireOption
    ) {
        PreparedMutation<StringOps.SetStringValue> prepared = operations.prepareSet(
                keyBytes,
                value,
                mode,
                expireOption,
        ...
        boolean completed = false;
        try {
            StringOps.SetStringValue preview = prepared.preview();
            MutationOutcome outcome = prepared.commit();
            completed = true;
