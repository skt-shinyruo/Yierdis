...
Yierdis 使用 JDK 25 `java.lang.foreign`。`YierdisFfmMemoryRuntime.allocateRegion(...)` 用 `Arena.ofShared()` 分配 region：region 可能创建于 bootstrap 期间，释放则发生在 DB owner thread。分配失败时关闭已创建的 arena，并抛出 `NativeCapacityExceededException`。`YierdisFfmRegion.close()` 关闭 arena，再调用 `YierdisFfmMemoryRuntime.onRegionClosed(...)` 扣减 live region 与 used bytes。
