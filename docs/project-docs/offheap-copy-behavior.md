...
RESP decode 后，`ByteArrayExecutionRequest` 保存稳定的 heap `byte[][]` argv，并持有脱离 Netty 对象的 memory lease。这是协议帧跨过 Netty decoder lifecycle 和 executor queue 的 ownership 边界。
