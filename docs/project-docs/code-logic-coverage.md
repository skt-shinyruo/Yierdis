...
| 类 | 关键方法/逻辑块 | 行为职责 | 关键分支/状态/不变量 | 线程/内存边界 | 相关测试 | 文档归属 | 覆盖状态 | 备注 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `DefaultYierdisEngine` | `prepare(...)` | `CommandSession` / request 到 command processor 的桥接 | preparation context、prepared reply shape、reply writer 延后创建 | engine 不直接触 DB internal | `DefaultYierdisEngineTest`, `DbEngineReadWriteBoundaryTest` | [`request-execution-flow.md`](./request-execution-flow.md), [`command-parsing-and-dispatch.md`](./command-parsing-and-dispatch.md), [`change-event-and-proxy-logic.md`](./change-event-and-proxy-logic.md) | `covered` | session contract 和 processor preparation 入口已补齐 |
...
| 类 | 关键方法/逻辑块 | 行为职责 | 关键分支/状态/不变量 | 线程/内存边界 | 相关测试 | 文档归属 | 覆盖状态 | 备注 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `CommandDefinition` | `parse(...)` / record accessors | 汇总命令 syntax、parser 和 preparer | arity 规则、MULTI policy、typed parse | command 元数据与 transport 无关 | `CommandDefinitionTest`, `CommandRegistryGuardTest` | [`commands-and-data-model.md`](./commands-and-data-model.md) | `covered` | 命令抽象和约束已有较完整说明 |
| `YierdisFastCommandProcessor` | `prepare(...)` | 命令查表、解析、事务分支、准备和错误翻译 | empty/unknown、parse error、MULTI queue | 只通过 API 访问 DB | `YierdisFastCommandProcessorPolicyTest`, `YierdisFastCommandProcessorRegistrationTest`, `YierdisFastCommandProcessorModuleTest`, `CommandVariantCoverageTest` | [`command-parsing-and-dispatch.md`](./command-parsing-and-dispatch.md), [`request-execution-flow.md`](./request-execution-flow.md) | `covered` | DB commit publication 不在 command processor 内推断 |
