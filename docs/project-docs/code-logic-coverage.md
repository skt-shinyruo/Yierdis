...
| 类 | 关键方法/逻辑块 | 行为职责 | 关键分支/状态/不变量 | 线程/内存边界 | 相关测试 | 文档归属 | 覆盖状态 | 备注 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ArchitectureDependencyRuleTest` | module dependency assertions | 防止 production 依赖方向回退 | command、storage、executor、execution API 的 forbidden edge | 架构护栏，不承载运行时状态 | [`ArchitectureDependencyRuleTest`](../../yierdis-tests/src/test/java/yier/bubu/redis/architecture/ArchitectureDependencyRuleTest.java) | [`module-architecture.md`](./module-architecture.md), [`testing-and-debugging.md`](./testing-and-debugging.md) | `covered` | 只冻结跨模块依赖方向，不冻结私有源码形状 |
| `RespBoundaryGuardTest` | retired protocol scan | 防止已删除 wire protocol 回到 production source/POM | active RESP boundary、retired artifact/package names | 架构护栏，不承载连接状态 | [`RespBoundaryGuardTest`](../../yierdis-tests/src/test/java/yier/bubu/redis/protocol/resp/RespBoundaryGuardTest.java) | [`protocol-reference.md`](./protocol-reference.md), [`testing-and-debugging.md`](./testing-and-debugging.md) | `covered` | 与真实 protocol integration tests 配合 |
| `YierdisDbArchitectureGuardTest` | DB/FFM isolation and composition assertions | 防止 DB 绕过 memory API 或恢复公开构造 | 无 FFM import、factory-only composition | 保护 DB 与 FFM 的编译期边界 | [`YierdisDbArchitectureGuardTest`](../../yierdis-tests/src/test/java/yier/bubu/redis/storage/memory/YierdisDbArchitectureGuardTest.java), `DbEngineReadWriteBoundaryTest` | [`db-internals.md`](./db-internals.md), [`testing-and-debugging.md`](./testing-and-debugging.md) | `covered` | 私有字段和历史 helper 形状由行为测试替代 |
