...
[`production-hardening-operations.md`](./production-hardening-operations.md) 说明 ingress、maxmemory 和有界回复的容量口径，result-unknown 关闭语义、graceful shutdown 和 soak。benchmark 对照使用 canonical title 和前八个共享 CSV 字段；通过与否由外部策略决定，文档本身不设性能门槛。
