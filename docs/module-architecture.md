...
从代码逻辑上看，`command-api/kernel/defaults` 的职责可以再具体化成：
...
`ExecutionRequest` / `ReplyWriter` 由 `yierdis-server-api` 拥有，不在 `custom-v1-wire`。旧 `core-contract` artifact 已从 active Maven 图中退休。
